...
```bash
node dist/cli.js gen background --desc "温暖书桌氛围的登录页背景，右侧留白放表单" --out ./demo/login-bg.webp --provider mock
```
...
核心规则很简单：页面需要真实视觉素材时调用 `wda`，而不是用代码手写复杂图形；素材已经存在时优先用 `regen`/`edit`，而不是每次都推倒重来。
